...
class VectorIndexer:
    ...
    def __init__(self, config: IndexerConfig = None, vector_store=None):
        """
        Initialize indexer.
        ...
        if not vector_store:
            ...
            self.vector_store = SupabaseVectorStore(db_url=db_url, collection_prefix="shielva_kb_")
            # We need to connect
        ...
        logger.info("VectorIndexer initialized", store_type="supabase")

    async def index_chunks(self, chunks: List[Dict[str, Any]], tenant_id: str, kb_id: str) -> List[IndexedChunk]:
        """
        ...
        for i in range(0, len(chunks), self.config.batch_size):
            ...
            try:
                batch_results = await self._index_batch(batch, tenant_id, kb_id)
                results.extend(batch_results)

            except Exception as e:
                logger.error("Batch indexing failed", batch_start=i, error=str(e))
        ...
        logger.info("Indexing complete", total=len(chunks), success=success_count, failed=len(chunks) - success_count)

        return results

    async def _index_batch(self, chunks: List[Dict[str, Any]], tenant_id: str, kb_id: str) -> List[IndexedChunk]:
        ...
        if self.vector_store:
            ...
            for chunk in chunks:
                ...
                doc = VectorDocument(
                    id=chunk["id"], content=chunk["content"], embedding=chunk["embedding"], metadata=metadata
                )
                documents.append(doc)

            # Upsert
            await self.vector_store.upsert(tenant_id=tenant_id, kb_id=kb_id, documents=documents)
            ...
            for chunk in chunks:
                results.append(
                    IndexedChunk(id=chunk["id"], document_id=chunk["document_id"], vector_id=chunk["id"], success=True)
                )
        else:
            # Mock indexing
            for chunk in chunks:
    ...
    async def delete_by_document(self, document_id: str, tenant_id: str, kb_id: str) -> int:
        """
        ...
        if self.vector_store:
            return await self.vector_store.delete_by_filter(
                tenant_id=tenant_id, kb_id=kb_id, filter={"document_id": document_id}
            )

        return 0

    async def delete_by_kb(self, tenant_id: str, kb_id: str) -> bool:
        """Delete all indexed content for a knowledge base."""
        if self.vector_store:
            return await self.vector_store.delete_collection(tenant_id=tenant_id, kb_id=kb_id)
