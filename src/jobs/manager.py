...
class JobManager:
    ...
    def create_job(
        self, tenant_id: str, kb_id: str, documents_count: int, webhook_url: Optional[str] = None
    ) -> IngestionJob:
        ...
        logger.info("Job created", job_id=job_id, tenant_id=tenant_id, kb_id=kb_id, docs=documents_count)
    ...
    def list_jobs(self, tenant_id: str, status: Optional[str] = None) -> List[IngestionJob]:
        ...
        return [
            ...
            if job.tenant_id == tenant_id and (status is None or job.status == status)
        ]
