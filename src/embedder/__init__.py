...
class EmbeddingClient:
    ...
    def __init__(self, config: EmbedderConfig = None):
        ...
        logger.info("EmbeddingClient initialized", provider=self.config.provider, model=self.config.model)
    ...
    async def _embed_cohere(self, texts: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            response = await client.embed(texts=texts, model=self.config.model, input_type="search_document")
    ...
    def _embed_mock(self, texts: List[str]) -> List[List[float]]:
        ...
        return [[random.random() for _ in range(self.config.dimension)] for _ in texts]
